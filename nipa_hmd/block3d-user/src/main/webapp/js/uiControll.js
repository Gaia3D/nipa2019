...
function contentsResize()
{
	...
	$('.craneList').height(hgt - ($('.craneList').outerHeight(true) - $('.craneList').height()) - 125);
	$('.blockLoadList').height(hgt - ($('.blockLoadList').outerHeight(true) - $('.blockLoadList').height()) - 125);
}
...
/***** NAV WRAP: 메뉴 *****/	
	...
	$("ul.nav li[data-nav]:not(:empty)").click(function() {
		...
		if($('#contentsWrap').css('display') == 'block') {
			...
			$('#selectMfgInd').parent('div').css('left','440px');
			$('#magoContainer').css('width','calc(100% - 470px)');
		} else {
			$('#contentsWrap').hide();
			$('#closeLeftBtn').toggle();
			
			$('#selectMfgInd').parent('div').css('left','65px');
			$('#magoContainer').css('width','calc(100% - 100px)');
		}
	});
...
/***** NAV WRAP: 맵노트 *****/	
    ...
    $("#searchTab > li").click(function() {
        var active = $(this).attr('data-nav');
        //var tpLayer = HMD.GIS.getLayerById('layer_tp');
        if($('#'+active).css("display") === "block") return;
        var display = $(this).toggleClass('on').hasClass('on');

        $("#searchTab > li").not($(this)).each(function() {
            $(this).removeClass('on');
            $('#' + $(this).attr('data-nav')).hide();
        });
        ...
        /*
         * if(active === 'tpSearch'){
            if($(':radio[name="tpMove"]:checked').length ===1) {
    ...
    $(".index").click(function() {
        $(".index button").toggleClass('on');
        $(".labelLayer").toggle();
    });
    $(".labelLayer .layerClose").click(function() {
